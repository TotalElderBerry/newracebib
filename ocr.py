...
def is_inside_contour(cnts, prev, current):
    ...
    print(f"first contour: {second_x+second_w} second contour: {first_x+first_w}")
    
    return second_x > first_x and second_y > first_y and \
           (second_x + second_w) < (first_x + first_w) and \
           (second_y + second_h) < (first_y + first_h)

image = cv2.imread('bib_images/roi_image_2.jpg')

is_image_small = is_small(image)
...
smallest_contour = min(cnts, key=cv2.contourArea)
largest_x,largest_y,largest_w,largest_h = cv2.boundingRect(largest_contour)
smallest_x,smallest_y,smallest_w,smallest_h = cv2.boundingRect(smallest_contour)
...
for contour in cnts:
    ...
    if x > 0 and y > 0 and y+h != scale.shape[0] and w > 10 and h > smallest_h:
        # if y+h != scale.shape[0]:
        if prev == -1 and curr == -1:
            curr = i-1
        else:
            prev = curr
            curr = i-1
        # print(f"Prev index: {prev} Current index: {i}")
        # if prev != curr:
        #     prev = curr
        #     curr = i
        ...
        new_width, new_height = 150, 150  # Adjust these dimensions as needed
...
# text = pytesseract.image_to_string(scale,lang="eng",config=custom_config)  # Perform OCR on the ROI
# print(f"Detected text: {text}")
